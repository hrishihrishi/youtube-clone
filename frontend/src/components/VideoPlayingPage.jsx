...
export default function VideoPlayingPage() {
    ...
    return (
        loading ? (
            ...
            <div className="flex flex-col lg:flex-row gap-6 p-4 lg:px-10" style={{ backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
                ...
                <div className="flex-grow lg:max-w-[calc(100%-400px)]">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>1.2m subscribers</p>
                            </div>
                            <button
                                onClick={() => setIsSubscribed(!isSubscribed)}
                                className="ml-4 px-4 py-2 rounded-full font-medium transition"
                                style={isSubscribed
                        ...
                        <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
                            <div className="flex items-center rounded-full" style={{ backgroundColor: 'var(--bg-surface)' }}>
                                {/* LIKE BUTTON */}
                                <button onClick={handleLike} className="flex items-center gap-2 px-4 py-2 hover:bg-[var(--bg-hover)] rounded-l-full" style={{ borderRight: '1px solid var(--border-color)' }}>
                                    {liked ? <AiFillLike size={20} /> : <AiOutlineLike size={20} />} {likes}
                                </button>
                                {/* DISLIKE BUTTON */}
                                <button onClick={handleDislike} className="px-4 py-2 hover:bg-[var(--bg-hover)] rounded-r-full">
                                    {disliked ? <AiFillDislike size={20} /> : <AiOutlineDislike size={20} />}
                                </button>
                            </div>
                            <button className="flex items-center gap-2 px-4 py-2 hover:bg-[var(--bg-hover)] rounded-full font-medium" style={{ backgroundColor: 'var(--bg-surface)' }}>
                                <AiOutlineShareAlt size={20} /> Share
                            </button>
                            {/* MORE BUTTON */}
                            <div >
                                <button className="p-2 hover:bg-[var(--bg-hover)] rounded-full" style={{ backgroundColor: 'var(--bg-surface)' }} onClick={onMoreClick}>
                                    <TfiMoreAlt />
                                </button>

                                {isMoreOpen && (
                                    <div className="absolute right-[10] mt-2 w-48 rounded-xl shadow-xl py-2 z-50" style={{ backgroundColor: 'var(--bg-primary)', border: '1px solid var(--border-light)' }}>
                                        <button
                                            onClick={() => {
                                                console.log("Save to playlist");
                                                setIsMoreOpen(false);
                                            }}
                                            className="w-full flex items-center gap-3 px-4 py-2 hover:bg-[var(--bg-hover)] transition text-sm font-medium"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="w-full flex items-center gap-3 px-4 py-2 hover:bg-[var(--bg-hover)] transition text-sm font-medium text-red-600"
                                        >
                                            <AiOutlineDelete size={20} /> Delete video
                                        </button>
                                        <button
                                            onClick={() => setIsEditModalOpen(true)}
                                            className="flex items-center gap-2 px-3 py-2 hover:bg-[var(--bg-hover)]  w-full text-sm font-medium transition"
                                        >
                                            <MdEdit size={20} />
                    ...
                    <div className="rounded-xl p-3 mt-4 text-sm hover:bg-[var(--bg-hover)] cursor-pointer transition" style={{ backgroundColor: 'var(--bg-surface)' }}>
                    ...
                    <div className="mt-6">
                        ...
                        <form onSubmit={addComment} className="flex gap-3 mb-8">
                            ...
                            <div className="flex-grow flex flex-col gap-2">
                                ...
                                {/* comment input */}
                                <input type="text" placeholder="Add a comment..." className="outline-none py-1 w-full" style={{ borderBottom: '1px solid var(--border-color)', backgroundColor: 'transparent', color: 'var(--text-primary)' }} value={newComment} onChange={(e) => setNewComment(e.target.value)} />

                                {/* comment buttons */}
                                <div className="flex justify-end gap-3 mt-1">
                                    <button type="button" onClick={() => setNewComment("")} className="px-4 py-2 hover:bg-[var(--bg-hover)] rounded-full font-medium">Cancel</button>
                                    <button type="submit" disabled={!newComment} className="px-4 py-2 bg-blue-600 text-white rounded-full font-medium disabled:opacity-50">Comment</button>
                                </div>
                        ...
                        <div className="flex flex-col gap-6">
                            {comments.map(c => (
                                ...
                                <div key={c.id} className="flex gap-3 text-sm">
                                    <div className="h-10 w-10 rounded-full shrink-0" style={{ backgroundColor: 'var(--bg-surface)' }} />
                                    <div className="flex-grow">
                                        <p className="font-bold">@{c.username?.toLowerCase() || 'guest'} <span className="font-normal ml-2" style={{ color: 'var(--text-muted)' }}>1 second ago</span></p>
                                        ...
                                        {editingCommentId === c.id ? (
                                            <div className="flex items-center gap-2 mt-1">
                                                ...
                                                <button
                                                    ...
                                                    className="px-3 py-1 rounded-full text-xs font-medium hover:bg-[var(--bg-hover)]"
                                        ...
                                        <div className="flex items-center gap-4 mt-2">
                                            ...
                                            <button className="p-2 hover:bg-[var(--bg-hover)] rounded-full" style={{ backgroundColor: 'var(--bg-surface)' }} onClick={() => onDeleteComment(c.id)}>
                ...
                <div className="lg:w-[400px] flex flex-col gap-4">
                    ...
                    {videoData?.map((v) => (
                        <div key={v._id} className="flex gap-2 cursor-pointer group" onClick={() => navigate(`/VideoPlaying?id=${v._id}`)}>
                            <div className="w-40 h-24 shrink-0 rounded-lg overflow-hidden" style={{ backgroundColor: 'var(--bg-surface)' }}>
                                <img
                            ...
                            <div className="flex flex-col pr-2 overflow-hidden">
                                ...
                                <p className="text-xs mt-1 uppercase" style={{ color: 'var(--text-secondary)' }}>{v.channel}</p>
                                <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>{v.views} views • {new Date(v.dateTime).toLocaleDateString()}</p>
                            </div>
                        </div>
                    ))}
                </div>
